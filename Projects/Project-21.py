...
if(N>cakeangle): #Only when N is greater tha cake angle cake can't be cut into pieces
    ...
    print("YES the cake will not cut in any piece of size %d"%N)
n=1 # start subratcrting the cake
for i in range(N):
    cakeangle-=n
    n+=1
    if(cakeangle<0):
        print("NO the cake would not cut into %d pieces such that no two of them are equal"%N)
        break
else:
        print("YES the cake would not cut into %d pieces such that no two of them are equal"%N)
